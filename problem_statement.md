...
**Update, same day: a human-driven session did exactly that.** A separate Claude.ai session with the Chrome extension enabled (the same method as the two existing manual batches) browsed 5 real threads via DuckDuckGo `site:reddit.com` search and extracted 19 new records — `_manual_reddit_batch_20260823.py`, none overlapping the 18 existing Reddit `source_url`s (verified directly against `extracted.jsonl` before writing anything). Reddit records: **18 → 37**. Every record was inspected for real, coherent content before being trusted — genuine Unicode curly quotes in one record's `verbatim_quote` initially looked like file corruption in a terminal print and were verified byte-for-byte to be valid UTF-8, not a data-integrity problem.

**Real, measured effect:** re-running dedupe → synthesize → score against the 597-record corpus moved **Q2 and Q3 from Weak to Medium** (32/597 = 5.4%, up from 19/578 = 3.3% — crossing the coverage function's 5% Medium threshold). Q1, Q4, Q5, Q6, Q10 all gained absolute count (1→7, 9→17, 5→15, 3→10, 8→14 respectively) but stayed Weak — real progress in the predicted direction (§21's "Reddit-volume-limited" diagnosis), just not yet enough to cross a confidence tier for those five. Q9's `segment_signal` count was unchanged at 10 — none of the 19 new records happened to state a segment attribute, a reminder that "more Reddit volume helps on average" is not "every batch helps every field."
...
## 25. The analysis layer — answering the 10 questions, not just counting them, 2026-08-23

**The gap.** The user asked whether the answers and analysis had been updated for the new reviews, or only the numbers. Checking honestly: **only the numbers, because there was no analysis layer at all.** Every question displayed a fill rate as its "finding" — Q1 ("Why do users add products to their wishlist?") read `save_motivation captured in 8/1234 extractions`. That is a measurement of how much data exists, not an answer. The engine could report how much it had found and never what it concluded.

**Written by hand, deliberately** (`web/src/lib/briefAnswers.ts`). The user's reasoning — that Groq is not up to this — is correct, and the failure mode is specific: synthesising across 1,234 records while holding the no-monetary-levers constraint, the friction/praise distinction, and the lens asymmetry is exactly the task where a one-shot model call returns fluent, unfalsifiable prose that is very hard to check. Hand-writing means every claim traces to records a reader can pull, and a wrong claim is an analyst error rather than an unattributable model artifact. The file says so in its header, and the UI says so above the answers — provenance is part of the deliverable, same discipline as §16c's research boundary.

**Scope held.** Per §13 the engine identifies, quantifies and compares; it does not propose solutions. The answers stop at what the corpus shows. Where users built their own workaround, that is reported as observed evidence of an unmet need — never as a feature recommendation. Solution design remains a later stage gated on the interviews, which is also what keeps the deck's scope claim credible.

**Structure** mirrors `psychologyResearch.ts` (§16c) — one TS module read by both the dashboard and the assistant's grounding context, so the two cannot drift. Each answer carries: the answer, the reasoning, named mechanisms, what the evidence actually is, and a caveat where the answer is thin or contaminated. Caveats are never omitted when one exists.

### What writing the answers surfaced that the numbers had hidden

**A real measurement error in Q4, now verified.** The question's coverage pools `deferral_trigger` with `delay_signal`. Reading the records showed those measure different things: all 51 `delay_signal` records come from App/Play and 86% use delivery-delay language ("17 days delayed", "supposed to be delivered on 14th Aug") — shipping complaints, not purchase postponement. All 9 `deferral_trigger` records are Reddit and genuinely about deferring a decision. **The headline "60/1234" overstates real deferral evidence roughly sixfold.** `QUESTION_FIELDS` should be corrected; flagged here rather than silently patched, since it changes a published number.

**The same contamination, smaller, in Q2/Q3 and Q9.** App/Play `hesitation_signal` is largely fragments and generic app complaints ("coupon not allowed", several one-word captures) rather than wishlist hesitation; `segment_signal` includes an entry reading only "Manipur". Both were reviewed and excluded from the written answers, with the exclusion stated rather than quietly applied.

**Substantive findings that only appear when the answers are read together:**
- **Price-watching is the through-line** — the main reason people save (Q1), the main reason they postpone (Q4), a major comparison mode (Q5), a driver of off-platform research (Q6), and the cause of a specific backfire where prices ROSE during the awaited sale (Q2). It is also the one lever this project is forbidden from pulling.
