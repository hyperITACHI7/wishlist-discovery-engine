...
- [x] **Reddit corpus grown 18 → 37 real records via a third manual batch (2026-08-23)** — `_manual_reddit_batch_20260823.py`, same Claude-Chrome-extension method as the two existing batches, 5 threads read via DuckDuckGo `site:reddit.com` search, deduped against all 18 existing `source_url`s. Real measured effect: Q2/Q3 crossed from Weak to Medium confidence (32/597, up from 19/578); Q1/Q4/Q5/Q6/Q10 all gained absolute count but stayed Weak. Every record was verified as real content (not fabricated, not corrupted) before being trusted. Full story, including a process gap where the browsing session initially reported the data as "live" when it hadn't actually written anywhere yet: `problem_statement.md` §21.
- [x] **The brief's 10 questions promoted to an always-visible section, with an honest per-question "why it's weak" line (2026-08-23)** — `QuestionCoverageDetail` moved out of a modal onto the Findings tab directly. `score.py`'s new `build_blocker()` splits the 8 non-Strong questions into "Reddit-volume-limited" (Q1, Q2, Q3, Q6, Q9, Q10 — the field fires at 6-72% per-record on the 18-record Reddit lens, just not enough Reddit records to clear the confidence bar) vs "rare everywhere" (Q4, Q5 — thin on every lens regardless of volume, by design handed to interviews). A background agent then tried to actually fix the Reddit-volume half by growing the corpus past 18 records via the same manual-browser method as the two existing batches — every retrieval path (direct fetch, web search, Jina Reader proxy, a Wayback Machine fallback) hit a real Reddit-side block, reproducing §9's finding that this tool environment can't reach Reddit even through a proxy. Zero records added, nothing fabricated. `problem_statement.md` §4's routing table was also corrected to match measured reality instead of the design-time estimate. Rationale: §21.
- [x] **Two corpus-native widgets added (2026-08-23)** — inspired by a real primary-research survey the user reviewed (N=28), checked against the engine's own real fill rates rather than assumed: "Decision Factors" (`decision_factors`, 141/578 records, 310 mentions, bucketed into 8 categories by a keyword-marker classifier — grounded in two passes against real leftover phrases, landing at 12.6% unclassified) and "What Happens After Purchase" (`post_purchase_outcome` rolled up corpus-wide, 174/560 App/Play records). Both ship as new keys directly inside `findings.json` (no new optional file), reusing `RankedBarChart` and one new small component, `OutcomeMixBar.tsx`. Framed honestly: `decision_factors` is general purchase-decision language, not exclusively wishlist-hesitation-specific. A companion idea — segmenting records into Browser/Considerer/Ready-Buyer personas by relabeling `intent_signal` — was evaluated and rejected: `not-determinable` is 78.5% of the corpus and means "no signal," not "browsing behavior," so the relabel would overstate confidence in an absence-of-data bucket. Rationale: `problem_statement.md` §20.
- [x] **Keyword Buzz and cross-tab matrix removed (2026-08-23)** — both were judged to add no real insight and are cut entirely: the pipeline's `keywords.py` script, `score.py`'s `build_cross_tabs` (and the `crossTabMatrices` key it wrote into `findings.json`), the two dashboard sections, their chart components, and their TypeScript types. `findings.json` was regenerated for the real corpus (578 records, 8 opportunity areas) without the removed key. This narrows the engine's output from the three views originally scoped in `problem_statement.md` §8 down to two — opportunity table and question coverage. Rationale: `problem_statement.md` §19.
